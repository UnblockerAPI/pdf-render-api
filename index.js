  ...
  try {
    ...
    if (!shouldForceRender && keyExists) {
      ...
      if (entryStillValid) {
        ...
        return res
          .status(200)
          .json({ success: true, pdfLocation: entry, fromCache: true });
      }
    ...
    if (contentTypeHeaderExists) {
      ...
      if (!contentType.includes("text/html")) {
        return res
          .status(400)
          .json({ success: false, reason: "NothingToRender" });
      }
      ...
      if (pdfDestination) {
        ...
        if (uploadResult.success) {
          ...
          return res
            .status(200)
            .json({ success: true, pdfLocation: uploadUrl, fromCache: false });
        } else {
          if (shouldDisplay) {
            res.status(200);
            res.set({
          ...
          return res
            .status(503)
            .json({ success: false, reason: "UploadFailed" });
        }
      } else {
        return res
          .status(500)
          .json({ success: false, reason: "PageLoadFailed" });
      }
    } else {
